...
Em  `/deploy/operator.yaml`, atualize  `image: docker.io/netapp/trident-operator:26.06.0` para refletir a localização do seu registro de imagens. Seu link:../trident-get-started/requirements.html#container-images-and-corresponding-kubernetes-versions["Imagens de Trident e CSI"] pode estar localizado em um registro ou em registros diferentes, mas todas as imagens CSI devem estar no mesmo registro. Por exemplo:

* `image: <your-registry>/trident-operator:26.06.0` se todas as suas imagens estiverem localizadas em um único registro.
* `image: <your-registry>/netapp/trident-operator:26.06.0` se a sua imagem Trident estiver localizada em um registro diferente das suas imagens CSI.
