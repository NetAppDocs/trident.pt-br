...
----
. Utilize  `helm install` e especifique um nome para sua implantação, como no exemplo a seguir, onde  `100..0` é a versão do Trident que você está instalando.
+
[listing]
----
...
| `enableConcurrency`  a| 
Permite operações simultâneas do controlador do Trident para melhorar o desempenho. Para obter mais informações sobre os drivers compatíveis, consulte o link: https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html#controller-scalability[Controller Scalability documentation].
| falso 
