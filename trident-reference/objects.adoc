...
| Trident.NetApp.io/snapshotServe | SnapshotServe | ontap-nas, ontap-nas-flexgroup, ontap-san 
...
| trident.netapp.io/skipRecoveryQueue | pularFilaDeRecuperação | ontap-nas, ontap-nas-economy, ontap-nas-flexgroup, ontap-san, ontap-san-economy 
|===
...
| AllowVolumeExpansion | booleano | verdadeiro, falso | Ative ou desative o suporte para aumentar o tamanho do PVC | ontap-nas, ontap-nas-economy, ontap-nas-flexgroup, ontap-san, ontap-san-economy, solidfire-san, azure-netapp-files | Mais de 1,11 anos 
...
| pularFilaDeRecuperação | cadeia de carateres | não | Durante a exclusão de um volume, ignore a fila de recuperação no armazenamento e exclua o volume imediatamente. 
|===
