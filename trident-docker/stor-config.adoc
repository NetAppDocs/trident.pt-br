...
Opção nas para acesso ao `.snapshot` diretório.
 a| 
Habilita ou desabilita o acesso ao `.snapshot` diretório.

Valores válidos: `true`, `false`. O comportamento padrão pode variar de acordo com a versão do ONTAP e do Trident. Defina explicitamente para garantir um comportamento consistente.
