...
=== Uso paralelo do driver Trident SAN/iSCSI e EBS-CSI

Se você pretende usar `ontap-san` drivers (por exemplo, iSCSI), a configuração de múltiplos caminhos necessária nos nós pode entrar em conflito com o driver CSI do Amazon Elastic Block Store (EBS).  Para garantir que o multipathing funcione sem impactar os discos do EBS no mesmo nó, você deve excluir o EBS da sua configuração de multipathing.  O seguinte é um exemplo de um `multipath.conf` arquivo que inclui as configurações necessárias do Trident e exclui discos EBS do multipathing:
