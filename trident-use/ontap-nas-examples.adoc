...
| `useREST` | Parâmetro booleano para usar APIs REST do ONTAP.  `useREST` Quando definido como `true`, o Astra Trident usará as APIs REST do ONTAP para se comunicar com o back-end. Quando definido como `false`, o Astra Trident usará chamadas ZAPI do ONTAP para se comunicar com o back-end. Esse recurso requer o ONTAP 9.11,1 e posterior. Além disso, a função de login do ONTAP usada deve ter acesso ao `ontap` aplicativo. Isso é satisfeito com as funções e `cluster-admin` predefinidas `vsadmin`. A partir da versão Astra Trident 24,06 e ONTAP 9.15,1 ou posterior, `userREST` é definida como `true` por padrão; altere `useREST` para para `false` para usar chamadas ONTAP ZAPI. | `true` Para ONTAP 9.15,1 ou posterior, caso contrário `false`. 
