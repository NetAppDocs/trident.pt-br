...
NOTE: Por padrão, as métricas do Trident são expostas na porta de destino. `8001` no `/metrics` ponto final. Essas métricas são *ativadas por padrão* quando o Trident é instalado. Você pode configurar o consumo de métricas do Trident via HTTPS na porta. `8444` também.
...
Você deve definir um alvo do Prometheus para coletar as métricas e obter informações sobre os backends gerenciados Trident , os volumes que ele cria e assim por diante. Verlink:https://prometheus-operator.dev/docs/developer/getting-started/["Documentação do operador Prometheus"^] .
