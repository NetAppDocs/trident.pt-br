...
Antes de provisionar volumes usando Trident, você precisa integrar seu backend de armazenamento ao Trident criando uma Trident Backend Configuration (TBC). A TBC define como Trident se conecta ao backend de armazenamento e fornece as informações necessárias para que Trident gerencie os recursos de storage.



[[create-a-trident-backend-configuration-tbc-for-fsx-for-ontap]]
== Criar uma configuração de backend Trident (TBC) para FSx for ONTAP

Existem duas maneiras de configurar o backend do Trident para usar um sistema de arquivos FSx for ONTAP: * Criar um StorageClass que utilize o recurso de Backend Automático. * Criar o backend e o StorageClass separadamente.

Os passos desta seção explicam como criar o backend separadamente. Para obter informações sobre como usar o recurso de Backend Automático, consulte a seção link:trident-fsx-storageclass-pvc.html#configure-a-storage-class-without-automatic-backend-configuration["Configurar StorageClass e PVC"] para mais informações.
...
| `storageDriverName` | Nome do driver de armazenamento. Não pode ser modificado após a criação. Para atualizar este parâmetro, você deve criar um novo backend. | `ontap-nas` `ontap-nas-economy`, , `ontap-nas-flexgroup` `ontap-san` , , , `ontap-san-economy` 
...
| `storagePrefix` | Prefixo usado ao provisionar novos volumes na SVM. Não pode ser modificado após a criação. Para atualizar esse parâmetro, você precisará criar um novo backend. | `trident`NOTA: Os parâmetros de configuração do backend que afetam a identidade do backend e a nomenclatura do volume, como  `storageDriverName` e  `storagePrefix`, não podem ser alterados após a criação. Para atualizá-los, você deve criar um novo backend. 
...
Você pode provisionar volumes SMB usando o  `ontap-nas` driver. Antes de concluir link:trident-fsx-storage-backend.html#fsx-for-ontap-driver-details["Integração do driver ONTAP SAN e NAS"], execute estas etapas: link:worker-node-prep.html#prepare-to-provision-smb-volumes["Prepare-se para provisionar volumes SMB"].
