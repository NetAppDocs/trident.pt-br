...
Você pode provisionar volumes SMB usando o `ontap-nas` motorista. Antes de concluir <<Integração de driver SAN e nas ONTAP>> Conclua estas etapas: link:https://docs.netapp.com/us-en/trident/trident-use/worker-node-prep.html#prepare-to-provision-smb-volumes["Prepare-se para provisionar volumes SMB"].
