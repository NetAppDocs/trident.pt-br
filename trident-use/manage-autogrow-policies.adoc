...
[role="lead"]
Após criar políticas de crescimento automático, você pode visualizá-las, atualizá-las e excluí-las. Você também pode monitorar quais volumes usam uma determinada política.
...
IMPORTANT: As alterações afetam todos os volumes que utilizam a política. Sempre que possível, teste as alterações primeiro em um ambiente que não seja de produção.
...
As políticas de crescimento automático usam proteção de finalizador para evitar exclusão acidental enquanto os volumes as utilizam ativamente.
...
----
. Se os volumes ainda utilizarem a política, a exclusão entra em um estado  `Deleting`. Verifique quais volumes são afetados:
+
[source, console]
----
tridentctl get autogrowpolicy production-db-policy -o yaml
...
| Os volumes utilizam a política | A política entra em  `Deleting` estado. Um finalizador bloqueia a conclusão até que todos os volumes sejam removidos. 
...
Autogrow é compatível com backends ONTAP. Autogrow também é compatível com Google Cloud NetApp Volumes (GCNV) para volumes NAS e SAN.
...
* *Volumes existentes (implantações brownfield):* Autogrow pode não funcionar para volumes existentes, mesmo que uma política de Autogrow válida seja aplicada. Essa limitação ocorre devido a uma migração em andamento das publicações de volumes. Para confirmar que a migração foi concluída, verifique os logs do controlador Trident em busca de  `"Migration completed"` mensagens.
...
A anotação do PVC sempre tem precedência. Se um PVC tiver a  `trident.netapp.io/autogrowPolicy` anotação, Trident usa esse valor independentemente do que o StorageClass especifica. Consulte link:automatic-volume-expansion.html#policy-precedence["Precedência de política"] para obter detalhes.
...
Defina a anotação PVC como  `"none"`. Essa configuração substitui qualquer política de nível de StorageClass para esse volume:
...
Trident interrompe a expansão do volume. Ele não cria mais solicitações de redimensionamento para esse volume, mesmo que o uso continue a aumentar além do `usedThreshold`.

Por exemplo, com essa política, Trident para de aumentar o volume depois que ele atinge 100 GiB:
...
Sim. Quando você atualiza uma política, todos os volumes que usam essa política adotam os novos parâmetros na próxima avaliação de crescimento. Não é necessário reiniciar os volumes.
...
As políticas utilizam proteção de finalização. Se os volumes ainda utilizarem a política, a exclusão entra em um estado  `Deleting` e aguarda até que você remova todos os volumes da política.
...
kubectl annotate pvc <pvc-name> \
  trident.netapp.io/autogrowPolicy-
----
Após você remover todos os volumes, Trident libera o finalizador e exclui a política.



=== Quais backends o autogrow suporta?

O Autogrow é compatível com backends ONTAP. Ele também é compatível com Google Cloud NetApp Volumes (GCNV) para volumes NAS e SAN.

O Autogrow é compatível com os protocolos NFS, iSCSI, FCP e NVMe. Volumes de bloco bruto NVMe requerem ONTAP 9.16.1 ou posterior.

Os volumes existentes em implantações brownfield podem exigir que a migração da publicação de volume seja concluída antes que o autogrow entre em vigor. Para verificar o status da migração, verifique os logs do controlador Trident:
