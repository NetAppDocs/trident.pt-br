...
[role="lead"]
Compreenda os requisitos, as opções de autenticação e as políticas de exportação para configurar um backend ONTAP com drivers ONTAP NAS. A partir da versão 25.10, NetApp Trident oferece suporte link:https://docs.netapp.com/us-en/ontap-afx/index.html["Sistema de armazenamento NetApp AFX"^]. NetApp AFX storage systems diferem de outros sistemas ONTAP (ASA, AFF e FAS) na implementação de sua camada de storage. Na configuração do backend do Trident, não é necessário especificar que seu sistema é AFX. Ao selecionar  `ontap-nas` como o  `storageDriverName`, o Trident detecta automaticamente os sistemas AFX.
