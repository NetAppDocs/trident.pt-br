...
Para obter mais informações sobre outras opções de configuração, consulte a <<first-link,Configuração avançada do backend e exemplos>> seção abaixo.
...
Você pode provisionar volumes SMB usando o  `ontap-nas` driver. No entanto, para isso, você deve concluir estas etapas: link:worker-node-prep.html#prepare-to-provision-smb-volumes["Prepare-se para provisionar volumes SMB"].



[[first-linkbackend-advanced-configuration-and-examples]]
== [[first-link]]Configuração avançada do backend e exemplos
