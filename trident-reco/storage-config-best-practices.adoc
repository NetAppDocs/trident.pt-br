...
O NetApp Trident suporta a clonagem de volumes ao usar o `ontap-nas` , `ontap-san` , e `solidfire-san` drivers de armazenamento. Ao usar o `ontap-nas-flexgroup` ou `ontap-nas-economy` Drivers, clonagem não é suportada. Criar um novo volume a partir de um volume existente resultará na criação de um novo snapshot.
