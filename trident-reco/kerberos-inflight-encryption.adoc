...
* *On-premise ONTAP* - Trident oferece suporte à criptografia Kerberos sobre NFSv3 e a conexões de clusters Red Hat OpenShift e Kubernetes upstream para volumes ONTAP locais.
...
* Certifique-se de que todos os volumes que você usa com criptografia Kerberos estejam configurados corretamente. Consulte a seção Configuração de Domínio da NetApp (página 13) do  https://www.netapp.com/media/16398-tr-3580.pdf["NetApp Enhancements e Guia de Práticas Recomendadas"^].
...
Configure a política de exportação com NFS, NFSv3 e protocolos de acesso.
