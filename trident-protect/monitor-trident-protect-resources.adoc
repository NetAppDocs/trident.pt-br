...
.Passos
. Integre o kube-State-metrics com Prometheus. Edite o arquivo de configuração Prometheus (`prometheus.yml`) e adicione as informações do serviço kube-State-metrics. Por exemplo:
+
.prometheus.yml: Integração do serviço kube-State-metrics com Prometheus
[source, yaml]
----
---
apiVersion: v1
...
----
. Configure Prometheus para rotear alertas para Alertmanager. Edite o arquivo de configuração Prometheus (`prometheus.yml`) e adicione a seguinte seção:
+
.prometheus.yml: Envie alertas para Alertmanager
[source, yaml]
----
alerting:
  alertmanagers:
...
O exemplo a seguir define um alerta crítico que é acionado quando o status do recurso personalizado de backup é definido como `Error` por 5 segundos ou mais. Você pode personalizar este exemplo para corresponder ao seu ambiente e incluir esse snippet YAML em seu `prometheus.yml` arquivo de configuração:

.Rules.yml: Defina um alerta Prometheus para backups com falha
[source, yaml]
----
rules.yml: |
  groups:
...
Você pode configurar o Alertmanager para enviar notificações para outros canais, como e-mail, PagerDuty, Microsoft Teams ou outros serviços de notificação especificando a respetiva configuração no `alertmanager.yml` arquivo.
...
.Alertmanager.yml: Envie alertas para um canal Slack
[source, yaml]
----
data:
  alertmanager.yml: |
