...
NOTE: Para definir um aplicativo com escopo de VM que protege máquinas virtuais (VMs) específicas do KubeVirt por nome em vez de namespaces inteiros, use o campo  `spec.includedVirtualMachines` no CR do aplicativo ou o sinalizador  `--virtual-machines` na CLI. Para obter detalhes, referência de campos e exemplos de YAML, consulte link:trident-protect-kubevirt-vms.html["Proteja máquinas virtuais KubeVirt com Trident Protect"].
...
. (_Opcional_) Se necessário, você pode adicionar filtragem de recursos à mesma CR para incluir ou excluir recursos específicos:
+
