...
Para saber mais sobre ganchos de execução, consultelink:../trident-protect/trident-protect-use-execution-hooks.html["Gerenciar os ganchos de execução do Trident Protect"] .
...
NOTE: Ao usar uma solicitação de alteração (CR), crie manualmente o namespace de destino antes de aplicar a CR. O Trident Protect cria namespaces automaticamente apenas quando se utiliza a CLI (linha de comando).
...
*** *spec.storageClassMapping*: (_Opcional_) Mapeia classes de armazenamento de origem para classes de armazenamento de destino para PVCs individuais dentro do mesmo AMR. Cada entrada requer um `source` e `destination` nome de classe de armazenamento. Quando definido, `storageClassMapping` substitui `storageClassName` completamente; PVCs sem uma entrada de origem correspondente usam a classe de armazenamento padrão do cluster, não `storageClassName`.
+
Ao utilizar  `storageClassMapping` em um AppMirrorRelationship CR, esteja ciente das seguintes restrições:

+
**** `storageClassMapping` substitui  `storageClassName` completamente. Quando ambos os campos estão definidos,  `storageClassName` é ignorado.
**** Os PVCs com classes de armazenamento de origem que não possuem entrada correspondente em `storageClassMapping` usam a classe de armazenamento padrão do cluster, não em `storageClassName`.
**** Cada entrada requer um valor  `source` e um valor  `destination`.
**** Alterar a classe de armazenamento de destino pode forçar a exclusão e recriação dos PVCs de destino existentes.
...
NOTE: Use  `--storageclass-mapping` para mapear classes de armazenamento de origem individuais para classes de armazenamento de destino. Especifique um ou mais pares  `source:destination` separados por vírgulas (por exemplo,  `"class-a:class-b,class-c:class-d"`). Use  `*:destination-class` como um caractere curinga para mapear todos os PVCs não mapeados para uma única classe de armazenamento de destino. Quando  `--storageclass-mapping` é definido,  `--storage-class` é ignorado.

+
*Exemplo usando uma única classe de armazenamento de destino (`--storage-class`:*
