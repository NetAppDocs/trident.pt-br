...
.Passos
. Se você link:../trident-get-started/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Trident instalado usando Helm"], pode usar  `helm upgrade trident netapp-trident/trident-operator --version 100.2602.0` para atualizar em uma única etapa. Se você não adicionou o repositório Helm ou não consegue usá-lo para atualizar:
+
...
.. Utilize o  `helm upgrade` comando onde  `trident-operator-26.02.0.tgz` reflete a versão para a qual você deseja atualizar.
+
[listing]
----
