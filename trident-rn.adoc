...
* O suporte à simultaneidade para os drivers ONTAP-NAS (somente NFS), ONTAP-SAN (iSCSI, FCP, NVMe) e Google Cloud NetApp Volumes agora está disponível para o público em geral (GA).
* Adicionado suporte para configuração automática de backend para Amazon FSx for NetApp ONTAP. Quando você cria um StorageClass que inclui os parâmetros necessários, NetApp Trident cria automaticamente o backend correspondente e VolumeSnapshotClass (se necessário).
* Adicionado suporte para diferentes nuvens do Microsoft Azure, como Azure Government e Azure China, e configuração personalizada de nuvem para backends do Azure NetApp Files.
* Escalabilidade aprimorada do controlador para volumes NVMe.
* Adicionado suporte para a funcionalidade de volume com crescimento automático. Você pode controlar o comportamento do crescimento automático definindo políticas personalizadas de volume com crescimento automático do NetApp Trident.
* Adicionado suporte para armazenamento em blocos do Google Cloud NetApp Volumes (SAN/iSCSI).
* Adicionado suporte para armazenamento em camadas automático para Google Cloud NetApp Volumes. Agora você pode otimizar os custos de armazenamento ao adicionar controles nativos do Kubernetes para configurar o ONTAP FabricPool armazenamento em camadas automático para Google Cloud NetApp Volumes por meio do NetApp Trident.
* Adicionado suporte para Kubernetes 1.35.
