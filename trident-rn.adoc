...
== O que há de novo na versão 26.06.1
...
* *Kubernetes*:
+
** Corrigida uma condição de corrida entre a publicação CSI e a movimentação de volume que podia anexar um volume usando informações de acesso desatualizadas.
** Corrigida uma condição de corrida em que NodeGetInfo poderia retornar rótulos de topologia vazios antes da conclusão do registro do nó, causando falhas no provisionamento com reconhecimento de topologia.
** Corrigida uma rara condição de corrida de desconexão NVMe que podia derrubar um subsistema compartilhado enquanto outro pod ainda estava sendo conectado.
** Corrigidas as falhas na publicação de clones de PVC NVMe (e o mapeamento incorreto do namespace pai) ao criar clones de um volume montado sob concorrência.
** Corrigido o problema de falhas na criação de snapshots para PVCs em backends ONTAP-SAN-Economy usando um nameTemplate.
...
* *Kubernetes*:
+
** Adicionada a opção  `storageAPITimeout` (segundos) na configuração do backend ONTAP para definir os tempos limite dos clientes de API REST e ZAPI por backend.
...
* *Kubernetes*:
+
** Adicionado suporte de disponibilidade geral (GA) para simultaneidade nos drivers ONTAP-NAS-Economy e ONTAP-SAN-Economy, elevando essa funcionalidade da Tech Preview, e adicionado suporte GA para simultaneidade nos drivers Azure NetApp Files (ANF) e SolidFire.
** Adicionado suporte para Kubernetes 1.36.
** Adicionada disponibilidade geral (GA) para volume com crescimento automático com os drivers NAS e SAN do Google Cloud NetApp Volumes (GCNV).
** Adicionada colocação balanceada do Unified ONTAP para seleção ideal de agregados ao provisionar novos volumes (link:https://github.com/NetApp/trident/issues/64["Problema #64"]).
** Adicionado suporte ONTAP AFX para o driver ONTAP-NAS-Flexgroup.
** Adicionada integração com o NetApp Shift Toolkit para simplificar as migrações de máquinas virtuais ao usar o Red Hat OpenShift Migration Toolkit for Virtualization.
** Adicionada a compatibilidade com Cloud Identity para o driver SAN do GCNV.
** Adicionado suporte a volumes subordinados para propagação do Trident Volume Publication (TVP) e fluxos de trabalho de volume com crescimento automático.
** Adicionado suporte para atualizações de volume e snapshots de grupo de volume ao executar Trident com a simultaneidade ativada.
** Adicionada limitação dinâmica de taxa gRPC para GCNV.
** Adicionada uma sonda de prontidão do controlador e ajustadas as configurações padrão de QPS e Burst da API do Kubernetes para o controlador e sidecars.
** Sequenciamento de operações de nós aprimorado ao separar as etapas de conexão, configuração de criptografia e formatação do sistema de arquivos.
** Os sistemas de arquivos raiz dos contêineres Trident foram montados como somente leitura para melhorar a segurança.
** Adicionada a remoção best-effort de snapshots para snapshots de volumes clonados em backends ONTAP quando o snapshot de origem de um volume clonado é excluído.
** Adicionada a funcionalidade de reconhecimento de PVCs de clones de backup para evitar que PVCs temporários criados por Veeam Kasten, Trilio e Cohesity fiquem presos na fila de recuperação de volumes do ONTAP.
** Melhoria no tratamento de ARNs da AWS para oferecer suporte a regiões especiais.
** Impedir a redução do tamanho dos Flexvols durante a operação de redimensionamento para o driver ONTAP-SAN-ECONOMY.
...
* *[Prévia Técnica]*: Adicionado suporte para importação de qtree ONTAP-NAS-Economy.
* *[Prévia Técnica]*: adicionado suporte para simultaneidade para os drivers ASA-R2 (SAN e NVMe).
* *[Prévia Técnica]*: Adicionado transporte HTTP GCNV ONTAP-mode para os drivers ONTAP-SAN e ONTAP-NAS.
...
* *Kubernetes*:
+
** Corrigidas as condições de corrida e o gerenciamento de estado para operações simultâneas de publicação/despublicação, clonagem, cache e atualização de backend.
** Corrigidas as condições de corrida na política de exportação e o tratamento simultâneo de publicação/despublicação para volumes subordinados e clones somente leitura nos drivers ONTAP-NAS e ONTAP-NAS-Economy.
** Correção da descoberta do ONTAP-NAS-Economy FlexVol após um failover do MetroCluster quando as políticas de snapshot diferem (link:https://github.com/NetApp/trident/issues/1082["Problema #1082"]).
** Corrigidas as novas tentativas de exclusão de volume ONTAP-NAS-Economy que interrompiam exclusões de qtree de longa duração (link:https://github.com/NetApp/trident/issues/1121["Problema #1121"]).
** Corrigido o comportamento de importação e redimensionamento do ONTAP-SAN e ONTAP-SAN-Economy, incluindo validação de fsType, tratamento de metadados de volume e comportamento do modo de crescimento automático.
** Corrigida uma condição de corrida de namespace NVMe/TCP durante a criação simultânea de volumes (link:https://github.com/NetApp/trident/issues/1089["Problema #1089"]).
** Corrigimos falhas no redimensionamento iSCSI que ocorriam quando os caminhos e portais disponíveis não correspondiam e melhoramos o comportamento em redimensionamentos repetidos.
** Corrigidas as falhas na exclusão de volumes ONTAP-SAN causadas por condições de volume ocupado.
** Correção no tratamento de erros REST do ONTAP e melhoria no comportamento de tempo limite do estado SVM.
** Correção no tratamento de pools zonais e grupos de hosts do GCNV SAN, normalização da conversão de identificadores de volume e normalização de solicitações de criação abaixo do tamanho mínimo.
** Correção na seleção do pool de capacidade nativo do GCNV para excluir pools no modo ONTAP.
** Correção do failover do KubeVirt para VMs que utilizam Trident Automated Workload Failover para acelerar o failover de VMs.
** Corrigidos problemas de RBAC em instalações gerenciadas pelo operador, incluindo ausência de ClusterRoles de nó e reconciliação incorreta de ClusterRole e ClusterRoleBinding durante as atualizações. Isso se aplica a instalações baseadas em Operator e Helm que usam o chart Trident Operator.
** Corrigidas diferenças persistentes de GitOps no TridentConfigurator e CRDs relacionados, omitindo valores de prioridade padrão e corrigindo a indentação do template YAML (link:https://github.com/NetApp/trident/issues/915["Problema #915"], link:https://github.com/NetApp/trident/issues/1120["Problema #1120"]).
** Correção na aplicação de patches em TridentVolumePublication CRDs durante uma atualização do Helm (link:https://github.com/NetApp/trident/issues/1151["Problema #1151"]).
** Corrigida a ordem de inicialização do CSI, iniciando o serviço gRPC do CSI antes das novas tentativas de registro do nó.
** Corrigidas as sobreposições de configuração de clonagem SMB segura durante operações simultâneas.
** Corrigidas as anotações ausentes no pod do controlador Trident após uma atualização (link:https://github.com/NetApp/trident/issues/1004["Problema #1004"]).
** Aprimorado o provisionamento lento de qtree em SVMs grandes (link:https://github.com/NetApp/trident/issues/1142["Problema #1142"]).
** Corrigido ControllerPublish para usar o tipo de sistema de arquivos de configuração de volume em vez de um tipo de sistema de arquivos padrão incorreto.
** Corrigido um problema em que dispositivos dm órfãos podiam causar falhas repetidas de CSI NodeUnstageVolume.
** Melhorada a limpeza de nós obsoletos e multipath, removendo artefatos fantasmas de multipath e recursos TridentNode obsoletos após a limpeza de nós excluídos de forma lógica.
...
* Adicionada a integração do OpenShift Service CA Operator para automatizar o gerenciamento de certificados TLS para o webhook de admissão do Trident Protect. Habilite definindo  `useOpenshiftServiceCA` como  `true` em seus valores do Helm. Para saber mais, consulte link:trident-protect/trident-protect-customize-installation.html["Personalize a instalação do Trident Protect"^].
* Adicionado suporte para backup e restauração de VMs KubeVirt na OpenShift Virtualization. Proteja todas as VMs em um namespace ou direcione VMs específicas pelo nome e restaure seletivamente VMs individuais a partir de um backup ou snapshot. Para saber mais, consulte link:trident-protect/trident-protect-kubevirt-vms.html["Proteja máquinas virtuais KubeVirt com Trident Protect"^].
* Adicionado suporte para transformações de recursos, permitindo que você adicione, remova e modifique atributos de recursos do Kubernetes durante o processo de restauração. Para saber mais, consulte link:trident-protect/trident-protect-resource-transformations.html["Modifique os recursos à medida que os restaura usando recursos personalizados"].
* Adicionado suporte para  `storageClassMapping` em AppMirrorRelationship (AMR), permitindo o mapeamento de classe de armazenamento para PVCs individuais dentro de uma única definição de AMR. Para saber mais, consulte link:trident-protect/trident-protect-use-snapmirror-replication.html["Replique aplicações usando NetApp SnapMirror e Trident Protect."^].
...
* Corrigido o problema de falha na restauração de máquina virtual entre clusters, que ocorria com um erro de webhook de admissão quando o namespace do aplicativo de origem não existia no cluster de destino.
* Corrigido o problema de falha na restauração seletiva de máquina virtual no local durante a etapa de criação do aplicativo devido a uma incompatibilidade de especificações com um recurso personalizado de aplicativo existente; o aplicativo existente agora é reutilizado.
* Corrigido o travamento de tarefas de restauração no local de máquina virtual em estado  `Running` quando tanto a máquina virtual quanto seu PersistentVolumeClaim eram excluídos; a ausência de um PersistentVolumeClaim agora é tratada como um erro fatal.
* Corrigido o problema em que a edição de uma programação de proteção existente acionava incorretamente um backup ou snapshot imediato.
* Corrigido o problema em que o finalizador de snapshot tentava buscar o recurso personalizado do aplicativo após ele ter sido excluído; agora, o finalizador ignora a tentativa quando o aplicativo não é encontrado.
* Capacidade de storage protegida corrigida aumentando significativamente após uma restauração no local de máquina virtual, utilizando cópia esparsa durante a etapa de cópia do PersistentVolumeClaim.
* Corrigido o problema em que o Trident Protect excluía o objeto de snapshot do Kubernetes antes de confirmar a exclusão do snapshot subjacente no ONTAP; agora, o finalizador aguarda a  `VolumeSnapshot` exclusão.
* Corrigido o problema em que os snapshots não prosseguiam quando uma operação de congelamento de máquina virtual falhava; as operações de congelamento e descongelamento agora continuam mesmo em caso de falha.
* Corrigido o problema de aplicativos de máquinas virtuais KubeVirt que recriavam incorretamente PersistentVolumeClaims durante as operações de reversão, ressincronização reversa e ressincronização de AppMirrorRelationship.




[[whats-new-in-26-02-1]]
== O que há de novo na versão 26.02.1
...
* *Kubernetes*:
* Corrigido o comportamento do SAN do Google Cloud NetApp Volumes (GCNV) para oferecer suporte a pools ZONAL do Flex Unified para cargas de trabalho em bloco (iSCSI). Agora, as configurações zonais e regionais são suportadas.
* Corrigido o tratamento do pool NAS UNIFIED do Google Cloud NetApp Volumes (GCNV) através da melhoria das operações de criação, validação, nomenclatura e pesquisa de volumes alternativos quando  `InternalID` está ausente.
* Corrigido um problema em que as operações de redimensionamento de dispositivos iSCSI podiam falhar quando os caminhos e portais disponíveis não correspondiam.
* Corrigido um problema intermitente que  `blkid` identificava incorretamente o tipo de sistema de arquivos para dispositivos LUKS, impedindo a criação de novos volumes.
* Corrigido um problema em que operações de publicação e preparação LUKS em grande escala podiam bloquear o controlador Trident. Agora, os usuários devem manter o controle das senhas LUKS associadas aos volumes.
* Corrigido um problema em que partições multipath iSCSI podiam aparecer como dispositivos fantasmas, impedindo que operações CSI `NodeUnstage` fossem bem-sucedidas.
* Corrigida uma condição de corrida no namespace NVMe/TCP durante a criação simultânea de volumes (Issue #1089).
* Corrigidas as operações simultâneas de clonagem de snapshots quando snapshots eram executados no mesmo segundo.
* Corrigido um problema que impedia as VMs KubeVirt usando  `dataVolumes` de realizarem failover com Trident Automated Workload Failover.
* Corrigido um erro que ocorria ao excluir um recurso personalizado inválido `TridentMirrorRelationship` ao usar o núcleo concorrente do Trident.
* Correção da indentação na  `tridentactionmirrorupdates`definição YAML do CRD do Kubernetes (Issue #1120).
* Corrigida a chamada da API REST de exclusão assíncrona do qtree (Problema nº 1121).
* Corrigido  `ControllerPublish` para usar o tipo de sistema de arquivos de configuração de volume em vez de um tipo de sistema de arquivos padrão incorreto.
* Corrigido o registro de inicialização do controlador para relatar corretamente o status do driver simultâneo.
* Correção no tratamento de backends suspensos no Trident concorrente.
* Melhoria no tratamento de ARNs da AWS para oferecer suporte a regiões especiais.
...
Dependências do Trident Protect atualizadas para corrigir vulnerabilidades de segurança:
...
* Tempo de execução Go: link:https://nvd.nist.gov/vuln/detail/CVE-2025-22871["CVE-2025-22871"^]
* Dependências adicionais: link:https://nvd.nist.gov/vuln/detail/CVE-2025-23109["CVE-2025-23109"^]
...
* *Kubernetes*:
+
** Adicionada disponibilidade geral (GA) para suporte à simultaneidade para ONTAP-NAS (somente NFS), ONTAP-SAN (iSCSI, FCP, NVMe) e Google Cloud NetApp Volumes (GCNV) drivers, graduando essa funcionalidade da versão de pré-visualização técnica. Consulte a link:https://docs.netapp.com/us-en/trident/trident-use/controller-scalability.html["Documentação sobre escalabilidade do controlador"] para mais informações.
** Adicionado suporte para volume com crescimento automático no Trident com base em políticas AutoGrow do Trident personalizadas e definidas pelo usuário. Consulte a link:https://docs.netapp.com/us-en/trident/trident-use/automatic-volume-expansion.html["Documentação de expansão automática de volume"] para mais informações.
** Concorrência de nós Trident aprimorada para melhorar a escalabilidade das operações de nós para volumes NVMe. Veja a link:https://docs.netapp.com/us-en/trident/trident-use/trident-nvme-rwx-subsystem-limits.html["Documentação de volumes NVMe"] para mais informações.
** Adicionado suporte para o armazenamento automático em camadas de volumes NAS do GCNV por meio de anotações de PVC (`tieringPolicy`, `tieringMinimumCoolingDays`), incluindo seleção de pool e herança de clones. Veja a link:https://docs.netapp.com/us-en/trident/trident-use/gcnv-auto-tiering.html["Configure o armazenamento em camadas automático para a documentação do Google Cloud NetApp Volumes"] para mais informações.
** Adicionado suporte para volumes de bloco SAN GCNV (iSCSI) usando o `google-cloud-netapp-volumes-san` driver, incluindo provisionamento, mapeamento de grupo de hosts por nó para acesso a LUN e operações de clonagem a partir de volume. Consulte alink:https://docs.netapp.com/us-en/trident/trident-use/gcnv-block.html["Documentação de configuração de blocos do Google Cloud NetApp Volumes"] para mais informações.
** Adicionado suporte para configuração automática de backend para Amazon FSx for NetApp ONTAP. Quando você cria um StorageClass que inclui os parâmetros necessários, Trident cria automaticamente o backend correspondente e o VolumeSnapshotClass, se necessário. Veja o link:https://docs.netapp.com/us-en/trident/trident-use/trident-fsx-auto-backend-configuration.html#add-additional-fsxn-file-systems["Documentação de configuração do backend do Amazon FSx for NetApp ONTAP"] para mais informações.
** Adicionado suporte para diferentes nuvens do Microsoft Azure, como Azure Government e Azure China, e configuração personalizada de nuvem para backends do Azure NetApp Files. Veja a link:https://docs.netapp.com/us-en/trident/trident-use/anf.html["Documentação de configuração do backend do Azure NetApp Files"] para mais informações.
** Adicionado suporte para Kubernetes 1.35. Veja a link:https://docs.netapp.com/us-en/trident/trident-get-started/requirements.html["Documentação de requisitos"] para mais informações.
...
* *Kubernetes*:
+
** Corrigido um problema em que a remoção da publicação de um clone somente leitura removia as regras de política de exportação do volume de origem nos drivers ONTAP-NAS, ONTAP-NAS-Economy e Google Cloud NetApp Volumes link:https://github.com/NetApp/trident/issues/1086["Problema #1086"].
** As imagens do kubectl foram alteradas para variantes leves baseadas em Alpine para evitar falhas de pull após a descontinuação pública de imagens pela Bitnami https://github.com/NetApp/trident/issues/1080["Problema #1080"].
** Corrigida a preservação das anotações da implantação existente durante a atualização do Trident link:https://github.com/NetApp/trident/issues/1004["Problema #1004"].
** Permitir clonagem entre diferentes classes de armazenamento se ambas as classes de armazenamento apontarem para o mesmo backend link:https://github.com/NetApp/trident/issues/1104["Problema #1104"].
...
** Melhorados os tempos limite de divisão de clones CSI e ONTAP para evitar condições de corrida com alguns aplicativos de backup link:https://github.com/NetApp/trident/issues/1098["Problema #1098"], link:https://github.com/NetApp/trident/issues/1100["Problema #1100"].
** Correção na supressão de mensagens de erro do LUKS link:https://github.com/NetApp/trident/issues/1069["Problema #1069"].
...
* Atualizado o pacote OpenTelemetry-Go para corrigir link:https://nvd.nist.gov/vuln/detail/cve-2026-24051["CVE-2026-24051"].
...
* Trident Protect agora desativa automaticamente os agendamentos de proteção e cancela as operações em andamento antes de uma restauração no local, e os reativa após a conclusão da restauração. Para saber mais, consulte link:trident-protect/trident-protect-restore-apps.html["Restaure aplicativos usando o Trident Protect."^].
* Adicionado o campo  `runImmediately` ao schedule CR e o sinalizador CLI  `--run-immediately` para acionar um backup ou snapshot imediato após a criação do agendamento. Para saber mais, consulte link:trident-protect/trident-protect-protect-apps.html#create-a-data-protection-schedule["Criar um cronograma de proteção de dados"^].
* Adicionada a possibilidade de especificar um nome personalizado para a aplicação restaurada usando o  `destinationApplicationName` campo no CR de restauração ou o  `--destination-app-name` parâmetro da CLI. Para saber mais, consulte link:trident-protect/trident-protect-restore-apps.html["Restaure aplicativos usando o Trident Protect."^].
...
** Adicionado suporte para failover automático de cargas de trabalho com desanexação forçada de volumes para os drivers ONTAP-NAS e ONTAP-NAS-Economy (excluindo SMB em ambos os drivers NAS), e para os drivers ONTAP-SAN e ONTAP-SAN-Economy. Verlink:https://docs.netapp.com/us-en/trident/trident-reco/force-detach.html["Automatizando o failover de aplicações com estado usando o Trident."^] .
...
** Adicionada uma opção para disponibilizar métricas do Prometheus com HTTPS, em vez de HTTP. Verlink:https://docs.netapp.com/us-en/trident/trident-use/monitor-trident.html["Monitore o Trident"^] .
** Adicionada uma opção `--no-rename` Ao importar um volume, o objetivo é manter o nome original, mas permitir que o Trident gerencie o ciclo de vida do volume. Verlink:https://docs.netapp.com/us-en/trident/trident-use/vol-import.html["Importar volumes"^] .
...
+
Verlink:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["Suporte para backup e anotações de agendamento."^] .



* Adicionada uma anotação ao CR de agendamento para configurar o tempo limite de vinculação do PVC, que será usado pelo CR de backup: `protect.trident.netapp.io/pvc-bind-timeout-sec` . Verlink:https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-protect-apps.html["Suporte para backup e anotações de agendamento."^] .
...
IMPORTANT: Para clientes que utilizam SolidFire, não atualizem para a versão 25.06.1 devido a um problema conhecido ao cancelar a publicação de volumes. A versão 25.06.2 será lançada em breve para corrigir esse problema.
